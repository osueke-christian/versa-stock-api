...
module.exports = {
    ...
    async add(req, res){
        try{
            ...
            if(!req.body) throw new Error('No parameter passed');
            if(!stock.product || !stock.quantity || !stock.price) throw new Error('Payload Incomplete');
            if(isNaN(stock.quantity) || isNaN(stock.price) || stock.quantity < 1 || stock.price < 1) throw new Error('Price & Quantity must be a number greater than 0');
    ...
    async update(req, res){
        try{
            ...
            if(!stock.product || !stock.quantity || !stock.price) throw new Error('Payload Incomplete');
            if(isNaN(stock.quantity) || isNaN(stock.price) || stock.quantity < 1 || stock.price < 1) throw new Error('Price & Quantity must be a number greater than 0');
            if(!stock.id) throw new Error('No stock specified');
